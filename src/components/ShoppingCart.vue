...
axios({
  ...
  (response) => {
    ...
    store.setcartitem(response.data.data);
  },
  (error) => {
...
<template>
  ...
  <br /><br />
  <div class="btn-right">
    <button @click="total" class="btn btn-success">Total</button> {{ total() }}
  </div>

  <router-link to="/checkout" class="btn btn-warning mt-2"
